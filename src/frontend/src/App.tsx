      ...
      <div className="bg-surface border border-steel-border rounded-2xl p-8 w-full max-w-sm shadow-2xl">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-xs text-muted-text">Set up your profile to get started</p>
          </div>
        </div>
        <label className="block text-xs font-medium text-muted-text mb-1.5">Your name</label>
        <input
          type="text"
          value={name}
...
function NavLink({
  ...
  const isActive = window.location.pathname === to || window.location.hash === `#${to}`;
  return (
    <Link
      to={to}
      onClick={onClick}
        ...
        <div className="max-w-screen-2xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-14 gap-4">
            ...
            <Link to="/" className="flex items-center gap-2 shrink-0">
              <svg width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg" aria-label="Auto Track Pro logo">
                <polygon points="18,2 33,10 33,26 18,34 3,26 3,10" fill="#1C1C2E" stroke="#F59E0B" strokeWidth="2"/>
                <text x="18" y="23" textAnchor="middle" fontFamily="Rajdhani, sans-serif" fontWeight="700" fontSize="13" fill="#F59E0B">ATP</text>
              </svg>
      ...
      <footer className="border-t border-steel-border bg-surface mt-auto">
        <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 py-6 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-muted-text">
          <div className="flex items-center gap-2">
            <svg width="28" height="28" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg" aria-label="Auto Track Pro logo">
              <polygon points="18,2 33,10 33,26 18,34 3,26 3,10" fill="#1C1C2E" stroke="#F59E0B" strokeWidth="2"/>
              <text x="18" y="23" textAnchor="middle" fontFamily="Rajdhani, sans-serif" fontWeight="700" fontSize="13" fill="#F59E0B">ATP</text>
            </svg>
            <span>
              © {new Date().getFullYear()} Auto Track Pro — Used Car Intelligence
            </span>
          </div>
...
const compareRoute = createRoute({ getParentRoute: () => rootRoute, path: '/compare', component: ComparisonPage });
...
const sharedWatchlistRoute = createRoute({ getParentRoute: () => rootRoute, path: '/shared-watchlist/$token', component: SharedWatchlistPage });
const depreciationRoute = createRoute({ getParentRoute: () => rootRoute, path: '/depreciation', component: DepreciationCurvePage });
const crossSearchRoute = createRoute({ getParentRoute: () => rootRoute, path: '/cross-search', component: CrossModelSearchPage });
const duplicatesRoute = createRoute({ getParentRoute: () => rootRoute, path: '/duplicates', component: DuplicateMergePage });
const ownershipCostRoute = createRoute({ getParentRoute: () => rootRoute, path: '/ownership-cost', component: OwnershipCostPage });
const sharedComparisonRoute = createRoute({ getParentRoute: () => rootRoute, path: '/shared-comparison', component: SharedComparisonPage });
const regionalRoute = createRoute({ getParentRoute: () => rootRoute, path: '/regional', component: RegionalBreakdownPage });
