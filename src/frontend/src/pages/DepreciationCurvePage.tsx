...
import { TrendingDown, Info, DollarSign, Calendar, BarChart2, Loader2 } from 'lucide-react';
...
function CustomTooltip({ active, payload, label, startingPrice }: TooltipProps) {
  if (!active || !payload?.length) return null;
  const value = payload[0]?.value as number;
  const drop = startingPrice - value;
  const dropPct = startingPrice > 0 ? ((drop / startingPrice) * 100).toFixed(1) : '0.0';

  return (
    <div className="bg-surface border border-steel-border rounded-lg p-3 shadow-xl text-xs">
      <p className="font-semibold text-amber font-rajdhani text-sm mb-1">{label}</p>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 text-muted-text text-xs mb-1">
        <span className={accent ? 'text-amber' : 'text-muted-text'}>{icon}</span>
        <span className="uppercase tracking-wider font-medium">{label}</span>
      </div>
      <p className={`text-xl font-bold font-rajdhani ${accent ? 'text-amber' : 'text-foreground'}`}>
        {value}
      </p>
...
export default function DepreciationCurvePage() {
  ...
  const liveData: ChartPoint[] = rawData && rawData.length > 0 ? buildChartData(rawData) : [];
  const isLiveMode = hasInputs && !isLoading;
  const chartData: ChartPoint[] = isLiveMode && liveData.length > 0 ? liveData : PLACEHOLDER_DATA;
    ...
    <div className="max-w-screen-xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <StatCard
          ...
          sub={isPlaceholder ? 'Sample projected value' : 'Most recent avg listing price'}
        />
        <StatCard
        ...
        {isLoading && hasInputs ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-3 text-center">
            ...
            <p className="text-xs text-muted-text max-w-xs leading-relaxed">
              Not enough listing data to project a depreciation curve — add more listings for{' '}
          ...
          <>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-semibold text-foreground font-rajdhani uppercase tracking-wide">
                {isPlaceholder ? 'Projected Value Over Time' : `${make} ${model} — Value Over Time`}
              </h2>
              <div className="flex items-center gap-2">
                ...
                >
                  {isPlaceholder ? 'Sample Data' : `${liveData.length} data points`}
                </span>
              </div>
            </div>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-4 text-xs text-muted-text border-t border-steel-border pt-3">
              ...
              <span className="flex items-center gap-1.5">
                <span
                  ...
                  style={{ background: 'rgba(251,191,36,0.35)', borderTop: '1px dashed rgba(251,191,36,0.5)' }}
                />
                50% Threshold
              </span>
              <span className="ml-auto italic">
                {make && model ? `${make} ${model}` : 'Enter make & model above to filter'}
              </span>
            </div>
          </>
        )}
        ...
        {isLoading && hasInputs ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {chartData.map((row, i) => {
                  const periodLoss = i === 0 ? 0 : chartData[i - 1].value - row.value;
                  const cumulativeLoss = startingPrice - row.value;
                  const retainedPct =
                    startingPrice > 0
                      ? ((row.value / startingPrice) * 100).toFixed(1)
                      : '100.0';
                  const livePoint = !isPlaceholder && rawData ? rawData[i] : null;
                  return (
                    <tr
                      key={`${row.label}-${i}`}
