...
import { ArrowLeft, Car, CheckCircle2, Loader2, Search } from "lucide-react";
...
export default function AddListingPage() {
  ...
  const lookupVIN = async () => {
    ...
    if (!/^[A-HJ-NPR-Z0-9]{17}$/i.test(trimmedVin)) {
      setVinError(
        "VIN must be exactly 17 alphanumeric characters (letters I, O, Q are not used).",
      );
      return;
    }
    ...
    try {
      ...
      if (displacement && cylinders) {
        engine = `${Number.parseFloat(displacement).toFixed(1)}L ${cylinders}-Cyl`;
      } else if (displacement) {
        engine = `${Number.parseFloat(displacement).toFixed(1)}L`;
      } else if (cylinders) {
      ...
      if (!make && !model) {
        setVinError(
          "Could not decode VIN. Please check the number and try again.",
        );
        return;
      }
      ...
      setVinError(
        "VIN lookup failed. Please check your connection and try again.",
      );
    } finally {
      setVinLoading(false);
    }
  ...
  const validate = (): boolean => {
    ...
    if (
      !form.year ||
      Number.isNaN(Number.parseInt(form.year)) ||
      Number.parseInt(form.year) < 1900 ||
      Number.parseInt(form.year) > new Date().getFullYear() + 2
    ) {
      newErrors.year = "Valid year required";
    }
    if (
      !form.price ||
      Number.isNaN(Number.parseInt(form.price)) ||
      Number.parseInt(form.price) <= 0
    ) {
      newErrors.price = "Valid price required";
    }
    if (
      form.mileage &&
      (Number.isNaN(Number.parseInt(form.mileage)) ||
        Number.parseInt(form.mileage) < 0)
    ) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const listing: ListingFormData = {
      ...
      mileage: BigInt(Number.parseInt(form.mileage) || 0),
      price: BigInt(Number.parseInt(form.price)),
      trim:
        form.trim.trim() +
        (form.engine.trim() ? ` | ${form.engine.trim()}` : ""),
  ...
  const vinFilledClass = vinFilled
    ? "ring-2 ring-amber-400 transition-all"
    : "";
    ...
    <main className="max-w-2xl mx-auto px-4 py-8">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Manually add a car listing to your tracker.
        </p>
      </div>
        ...
        <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-4 space-y-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground ml-1">
              — enter a VIN to auto-populate form fields
            </span>
          </div>
          <div>
