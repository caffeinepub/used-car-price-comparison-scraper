...
function calcAnnualFuel(fuelType: FuelType, annualMiles: number): number {
  switch (fuelType) {
    case "Gas":
      return (annualMiles / 28) * 3.5;
    case "Hybrid":
      return (annualMiles / 45) * 3.5;
    case "Electric":
      return (annualMiles / 3.5) * 0.14;
    case "Diesel":
      return (annualMiles / 35) * 4.0;
  }
}
...
function calcMonthlyLoan(
  ...
  const n = termMonths;
  return (loanAmount * r) / (1 - (1 + r) ** -n);
}

function calcAll(inputs: CalcInputs): CalcResults {
  const annualFuel = calcAnnualFuel(inputs.fuelType, inputs.annualMiles);
  const annualMaintenance = calcAnnualMaintenance(
    inputs.year,
    inputs.currentMileage,
  );
  const annualInsurance = calcAnnualInsurance(
  ...
  const totalAnnual =
    annualFuel + annualMaintenance + annualInsurance + annualLoan;
...
function ResultCard({ icon, label, value, sub, highlight }: ResultCardProps) {
  ...
  return (
    <div
      className={`rounded-xl border p-5 flex flex-col gap-2 ${c.border} ${c.bg}`}
    >
      <div
        className={`w-9 h-9 rounded-lg flex items-center justify-center ${c.iconBg}`}
      >
      ...
      <div
        className={`text-2xl font-bold font-display tracking-tight ${c.text}`}
      >
        {value}
      </div>
        ...
        <tbody>
          {rows.map(({ years, operatingCost, totalCost }) => (
            ...
            >
              ...
              <td className="px-4 py-3 text-right text-amber font-medium">
                {fmt(operatingCost)}
              </td>
...
export default function OwnershipCostPage() {
  ...
  const breakdownSegments: BreakdownSegment[] = [
    {
      label: "Fuel",
      value: results.annualFuel,
      color: "text-amber",
      bg: "bg-amber",
    },
    {
      label: "Maintenance",
      value: results.annualMaintenance,
      color: "text-blue-400",
      bg: "bg-blue-500",
    },
    {
      label: "Insurance",
      value: results.annualInsurance,
      color: "text-emerald",
      bg: "bg-emerald",
    },
    {
      label: "Loan",
      value: results.monthlyLoan * 12,
      color: "text-rose-400",
      bg: "bg-rose-500",
    },
  ];
  ...
  const annualOperating =
    results.annualFuel + results.annualMaintenance + results.annualInsurance;
    ...
    <div className="max-w-screen-xl mx-auto px-4 py-8 space-y-6">
      ...
      {(search.make || search.model) && (
        <div className="flex items-start gap-2 rounded-lg bg-amber/5 border border-amber/20 px-4 py-3">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            ...
            {search.mileage && (
              <span> · {Number(search.mileage).toLocaleString()} mi</span>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              >
                <input
                  ...
                  onChange={(e) =>
                    set("currentMileage", Number(e.target.value))
                  }
                  min={0}
                  step={1000}
                  className={inputCls}
          ...
          <section className="bg-surface border border-steel-border rounded-xl p-5 space-y-4">
            <div className="flex items-center gap-2 pb-1 border-b border-steel-border/60">
              ...
              <span className="text-xs text-muted-foreground ml-auto">
                Set loan amount to 0 if paying cash
              </span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <InputField
              ...
              <InputField
                label="Loan Term"
                sublabel="months"
                htmlFor="oc-loan-term"
              >
                <select
                  id="oc-loan-term"
                  value={inputs.loanTerm}
                  onChange={(e) =>
                    set("loanTerm", Number(e.target.value) as LoanTerm)
                  }
                  className={inputCls}
                >
                  {LOAN_TERMS.map((t) => (
                    <option key={t} value={t}>
                      {t} months ({(t / 12).toFixed(1)} yrs)
                    </option>
                  ))}
                </select>
              </InputField>
        ...
        <div className="space-y-5 xl:sticky xl:top-20">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <ResultCard
              ...
              sub={
                inputs.loanAmount > 0
                  ? `${inputs.loanTerm}mo @ ${inputs.loanRate}%`
                  : "No financing"
              }
              highlight="rose"
            />
          ...
          <div className="bg-surface border border-amber/30 rounded-xl p-5 space-y-3">
            ...
            <div className="space-y-2">
              ...
              <div className="border-t border-steel-border/60 pt-2 mt-2 flex justify-between items-baseline">
                ...
                <span className="text-2xl font-bold font-display text-amber">
                  {fmtDec(results.totalMonthly)}
                </span>
              </div>
              <div className="flex justify-between items-baseline">
                ...
                <span className="text-xl font-bold font-display text-amber">
                  {fmt(results.totalAnnual)}
                </span>
              </div>
            </div>
          </div>
      ...
      <div className="flex items-start gap-2 rounded-lg bg-muted border border-steel-border/40 px-4 py-3">
        ...
        <p className="text-xs text-muted-foreground leading-relaxed">
          ...
          Estimates are approximate and based on national averages. Actual fuel
          economy, insurance premiums, and maintenance costs vary by driver
          behavior, location, insurer, and vehicle condition. This calculator is
