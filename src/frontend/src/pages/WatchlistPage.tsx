...
export default function WatchlistPage() {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    await addToWatchlist.mutateAsync({ make: make.trim(), model: model.trim(), note: note.trim() || undefined });
    setMake('');
    setModel('');
    setNote('');
    setShowForm(false);
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center p-8 bg-surface border border-steel-border rounded-xl max-w-sm">
          ...
          <h2 className="text-lg font-bold text-foreground mb-2 font-rajdhani">Sign In Required</h2>
          <p className="text-sm text-muted-text">Please sign in to view and manage your watchlist.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-text">Track cars you're interested in</p>
            </div>
          </div>
        ...
        {showForm && (
          ...
          >
            <h3 className="text-sm font-semibold text-foreground">Add to Watchlist</h3>
        ...
        {isLoading ? (
          <div className="space-y-3">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="p-4 rounded-lg bg-surface border border-steel-border">
            ...
            {watchlist.map((entry) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {entry.note && (
                    <p className="text-xs text-muted-text mt-0.5">{entry.note}</p>
                  )}
                  <p className="text-xs text-muted-text mt-1">
                    Added {new Date(Number(entry.createdAt) / 1_000_000).toLocaleDateString()}
                  </p>
                </div>
                <button
