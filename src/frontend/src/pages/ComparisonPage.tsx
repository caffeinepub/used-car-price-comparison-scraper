...
function PriceDropAnnotationDot({
  cx = 0,
  cy = 0,
  event,
...
function makePriceDropDotRenderer(
...
export default function ComparisonPage() {
  ...
  const { data: models = [], isLoading: modelsLoading } =
    useGetDistinctModels(selectedMake);
  ...
  const { data: priceDropEvents = [], isLoading: eventsLoading } =
    usePriceDropEvents(selectedMake, selectedModel);
          ...
          {(!selectedMake || !selectedModel) && (
            <div className="card-panel p-12 text-center">
              ...
              <p className="text-muted-foreground text-sm">
                Choose a make and model above to view price history, trends, and
                market analysis.
              </p>
            </div>
          )}
