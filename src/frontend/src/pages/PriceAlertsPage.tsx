...
import { Bell, LogIn, Plus, Trash2, TrendingDown } from "lucide-react";
    ...
    <div className="card-panel">
      ...
      {listings.length > 0 ? (
        <div>
          ...
          <div className="space-y-1.5">
            {listings.slice(0, 3).map((l: any) => (
              <div
                key={l.id}
                className="flex items-center justify-between text-xs p-2 rounded bg-background border border-steel-border"
              >
        ...
        <p className="text-xs text-muted-foreground">
          No listings below target price yet.
        </p>
      )}
    </div>
  );
...
export default function PriceAlertsPage() {
  ...
  const handleSetAlert = async (e: React.FormEvent) => {
    ...
    try {
      await setAlertMutation.mutateAsync({
        make: make.trim(),
        model: model.trim(),
        targetPrice: price,
      });
      toast.success(
        `Alert set for ${make} ${model} at $${price.toLocaleString()}`,
      );
      setMake("");
      setModel("");
      setTargetPrice("");
    ...
    return (
      <main className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <p className="text-muted-foreground text-sm">
          Please sign in to manage price alerts.
        </p>
      </main>
    );
  }
    ...
    <main className="max-w-4xl mx-auto px-4 py-8">
      ...
      {isLoading ? (
        ...
        <div className="card-panel text-center py-12">
          ...
          <p className="text-muted-foreground text-sm">
            Add an alert to be notified when listings drop below your target
            price.
          </p>
        </div>
      ) : (
