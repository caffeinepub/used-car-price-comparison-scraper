...
import { X, Download, Save, Trash2, Check } from 'lucide-react';
import {
  useGetFilterPresets,
  useSaveFilterPreset,
...
import { downloadCSV, buildCSV } from '../utils/csvExport';

interface ExportFilterPanelProps {
  onClose: () => void;
}

export default function ExportFilterPanel({ onClose }: ExportFilterPanelProps) {
  const [presetName, setPresetName] = useState('');
  const [savedMsg, setSavedMsg] = useState('');
  const [renamingId, setRenamingId] = useState<bigint | null>(null);
  const [renameValue, setRenameValue] = useState('');

  const { data: presets = [] } = useGetFilterPresets('dashboard');
  ...
  const handleSavePreset = async () => {
    ...
    await savePreset.mutateAsync({ name: presetName.trim(), filterJson: '{}', presetType: 'dashboard' });
    setPresetName('');
  ...
  const handleRenamePreset = async (id: bigint) => {
    ...
    await updatePreset.mutateAsync({ id, name: renameValue.trim(), filterJson: '{}', presetType: 'dashboard' });
    setRenamingId(null);
    setRenameValue('');
  };

  const handleDeletePreset = async (id: bigint) => {
    await deletePreset.mutateAsync({ id, presetType: 'dashboard' });
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-surface border border-steel-border rounded-2xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-steel-border">
          <h2 className="text-lg font-semibold text-foreground">Export Dashboard</h2>
            ...
            {presets.length === 0 ? (
              ...
              <div className="space-y-2 max-h-40 overflow-y-auto">
                {(presets as any[]).map((preset: any) => (
                  <div key={String(preset.id)} className="flex items-center justify-between bg-background/40 rounded-lg px-3 py-2">
                    {renamingId === preset.id ? (
                      <div className="flex items-center gap-2 flex-1">
                        <input
                          ...
                          autoFocus
                        />
                        <button onClick={() => handleRenamePreset(preset.id)} className="text-emerald-400 hover:text-emerald-300">
